...
func TestBoundExtend(t *testing.T) {
	bound := Bound{Min: [2]float64{0, 0}, Max: [2]float64{3, 5}}

	if r := bound.Extend(Point{2, 1}); !r.Equal(bound) {
		t.Errorf("extend incorrect: %v != %v", r, bound)
	}

	answer := Bound{Min: [2]float64{0, -1}, Max: [2]float64{6, 5}}
...
func TestBoundUnion(t *testing.T) {
	b1 := Bound{Min: [2]float64{0, 0}, Max: [2]float64{1, 1}}
	b2 := Bound{Min: [2]float64{0, 0}, Max: [2]float64{2, 2}}

	expected := Bound{Min: [2]float64{0, 0}, Max: [2]float64{2, 2}}
...
func TestBoundContains(t *testing.T) {
	bound := Bound{Min: [2]float64{-2, -1}, Max: [2]float64{2, 1}}
...
func TestBoundIntersects(t *testing.T) {
	bound := Bound{Min: [2]float64{0, 2}, Max: [2]float64{1, 3}}
	...
	cases := []struct {
	...
	}{
		{
			...
			bound:  Bound{Min: [2]float64{5, 7}, Max: [2]float64{6, 8}},
			result: false,
		},
		{
			name:   "outside, top left",
			bound:  Bound{Min: [2]float64{-6, 7}, Max: [2]float64{-5, 8}},
			result: false,
		},
		{
			name:   "outside, above",
			bound:  Bound{Min: [2]float64{0, 7}, Max: [2]float64{0.5, 8}},
			result: false,
		},
		{
			name:   "over the middle",
			bound:  Bound{Min: [2]float64{0, 0.5}, Max: [2]float64{1, 4}},
			result: true,
		},
		{
			name:   "over the left",
			bound:  Bound{Min: [2]float64{-1, 2}, Max: [2]float64{1, 4}},
			result: true,
		},
		{
			name:   "completely inside",
			bound:  Bound{Min: [2]float64{0.3, 2.3}, Max: [2]float64{0.6, 2.6}},
			result: true,
		},
	}
	...
	a := Bound{Min: [2]float64{7, 6}, Max: [2]float64{8, 7}}
	b := Bound{Min: [2]float64{6.1, 6.1}, Max: [2]float64{8.1, 8.1}}

	if !a.Intersects(b) || !b.Intersects(a) {
		t.Errorf("expected to intersect")
	}

	a = Bound{Min: [2]float64{1, 2}, Max: [2]float64{4, 3}}
	b = Bound{Min: [2]float64{2, 1}, Max: [2]float64{3, 4}}
...
func TestBoundIsEmpty(t *testing.T) {
	...
	bound := Bound{Min: [2]float64{1, 1}, Max: [2]float64{0, 2}}
	if !bound.IsEmpty() {
		t.Error("expected true, got false")
	}

	// negative/malformed area
	bound = Bound{Min: [2]float64{1, 3}, Max: [2]float64{1, 2}}
	if !bound.IsEmpty() {
		t.Error("expected true, got false")
	}
}
...
func TestBoundPad(t *testing.T) {
	bound := Bound{Min: [2]float64{1, 1}, Max: [2]float64{2, 2}}

	bound = bound.Pad(0.5)
	if bound.Min != [2]float64{0.5, 0.5} {
		t.Errorf("incorrect min: %v", bound.Min)
	}

	if bound.Max != [2]float64{2.5, 2.5} {
...
func TestBoundIsZero(t *testing.T) {
	bound := Bound{Min: [2]float64{1, 2}, Max: [2]float64{1, 2}}
	if bound.IsZero() {
